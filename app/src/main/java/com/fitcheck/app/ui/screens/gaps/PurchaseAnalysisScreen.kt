...
fun PurchaseAnalysisScreen(categoryName: String, onBack: () -> Unit, vm: WardrobeGapsViewModel = viewModel()) {
    ...
    val title = gap?.title ?: categoryName.lowercase().replaceFirstChar { it.uppercase() }
    val price = gap?.price ?: "Price not set"
    ...
    LazyColumn(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 24.dp)) {
        ...
        item { AnalysisCard("WARDROBE COMPATIBILITY", "Works with ${gap?.compatible ?: 0} existing items", "Unlocks approximately ${gap?.newOutfits ?: 0} new outfit combinations.") }
        item { AnalysisCard("EXPECTED USAGE", "~${gap?.expected ?: 12} wears/year projected", "Estimated cost per wear: ${if (price.contains("7,500")) "₹625" else "₹350"} after 1 year.") }
